...
\title{Evaluation Artifacts, Not Algorithmic Limits: The CIFAR-10 Gap in Byzantine-Robust, Differentially Private Federated Learning}
...
significance testing at up to $n{=}10$ seeds. The clipping-ceiling
miscalibration is ruled out as a mechanistically-proven no-op (the ceiling
provably never binds, confirmed exactly at $n{=}10$, Wilcoxon $p{=}1.000$); a
four-point learning-rate sweep and a four-point normalization sweep, both at
$n{=}3$, show no beneficial trend, reported as such rather than as a
significance-tested null; momentum cold-start shows a small, non-significant
positive direction at $n{=}10$ ($p \geq 0.43$); and the clipping floor shows the
largest such direction, also non-significant (Wilcoxon $p{=}0.064$), including
when its hand-tuned constant is replaced by a tail-index-calibrated variant
designed specifically to close the gap between this line of work's own
heavy-tail measurements and its clipping hyperparameter ($p \geq 0.275$).
...
mechanistically-proven no-op; learning-rate transfer and normalization show no
beneficial trend across small, $n{=}3$ sweeps (not significance-tested nulls);
momentum cold-start shows a small, non-significant positive direction at
$n{=}10$; and the clipping floor shows the same direction, more strongly but
still short of significance. None of the five, on the evidence gathered, is a
complete explanation for the collapse. We then
...
normalization showing no beneficial trend across small ($n{=}3$) sweeps;
momentum cold-start showing a small, non-significant effect at $n{=}10$; and a
missing clipping floor's effect real in direction but not statistically
significant. (2) A tail-index-calibrated clipping mechanism, grounded in the
...
a completed scale-up. We define the CONFIRMATORY family as the seven $p$-values
computed for mechanisms with $n{=}10$ (the two floor mechanisms, clean and IPM;
the growing-schedule ceiling's clean condition; the momentum warm-start, clean
and IPM) and apply a Bonferroni correction across that family
(family size 7, corrected $\alpha{=}0.05/7{\approx}0.00714$):
\begin{center}
\small
\begin{tabular}{lc}
\toprule
...
\end{tabular}
\end{center}
None of the seven approaches even the uncorrected $\alpha{=}0.05$, so none
survive Bonferroni correction either -- the correction does not change any
audit conclusion in this paper, but we report it explicitly rather than only
asserting informally that it would not matter. The $n{=}3$-only mechanisms
(adaptive-quantile and EVT-quantile ceilings, the learning-rate and
GroupNorm sweeps) are excluded from this family: the first two are
mechanistically-established no-ops rather than significance tests (see
Section~\ref{sec:clip}), and the latter two are explicitly reported as
small-sample sensitivity sweeps, not hypothesis tests, so including them in a
correction would misrepresent what they are. Section~\ref{sec:resolution}'s
own result ($p{=}0.00195$, Section~\ref{sec:resolution}) is treated as a
separate, single confirmatory test of a different hypothesis (does the
resolution mechanism recover accuracy, not does an audit mechanism) and is not
diluted by this family; it survives Bonferroni correction against a family of
any plausible size considered in this paper by a wide margin.
...
$T{=}80$, $n{=}3$) does not reproduce this recovery: mean accuracy is
18.75\% at $\gamma{=}0.1$, 13.87\% at $\gamma{=}0.05$, 12.08\% at $\gamma{=}0.02$,
and 12.66\% at $\gamma{=}0.01$ -- flat to slightly worse as $\gamma$ decreases,
the opposite of the single-worker result. We report this as a four-point
sensitivity sweep at $n{=}3$, not a significance-tested null result: the sample
is too small to formally test, and we do not claim it rules learning-rate
transfer out on its own. What it shows is that the single-worker recovery does
not carry over once clipping and double-momentum are active, which is
sufficient to conclude learning-rate transfer is not, by itself, a complete
explanation for the collapse -- it does not establish that retuning $\gamma$
has no effect under any condition.
...
compose with it correctly. Across the same $\gamma$ grid, $n{=}3$: mean accuracy
is 10.43\% at $\gamma{=}0.1$ (\emph{worse} than the unnormalized architecture's
18.75\%), 11.15\% at $\gamma{=}0.05$, 16.34\% at $\gamma{=}0.02$, and 19.03\% at
$\gamma{=}0.01$. As with the learning-rate sweep above, this is a four-point,
$n{=}3$ sensitivity sweep, not a significance-tested claim of no effect. The
best GroupNorm cell (19.03\% at $\gamma{=}0.01$) sits within the same range as
the best unnormalized cell (18.75\% at $\gamma{=}0.1$) rather than clearly
above it, and at the algorithm's own headline $\gamma$, GroupNorm is
numerically worse. We read this as evidence against normalization being a
straightforward fix at these settings, not as a formally established null.
...
CIFAR-10 under IPM 7.9--8.3 (all 3 pilot-scale seeds) -- sits above 2, so mapping $\alpha$ into the theorem's required
$p \in (1,2]$ range
...
(17.13\%$\,\pm\,$3.15\% both, Wilcoxon $p{=}1.000$); MNIST, $n{=}10$, both
conditions, $p{=}1.000$.
...
construction, so we report it with the appropriate caveat: at $n{=}3$ (CIFAR-10,
both conditions, $q \in \{0.01, 0.05, 0.2\}$), accuracy remains within
0.001--0.006 of the fixed-threshold baseline on every one of the 18 seed/cell
combinations (e.g.\ clean, $q{=}0.05$: 18.64\% vs.\ baseline 18.75\%) --
consistent with no effect, but at
this sample size we treat it as no detected effect rather than a
significance-tested null.
...
(Wilcoxon $p{=}0.447$); IPM from 16.26\%$\,\pm\,$2.26\% to
...
16.26\%$\,\pm\,$2.26\% to 17.97\%$\,\pm\,$3.40\% ($p{=}0.064$). Neither reaches
...
\midrule
Learning-rate retune ($\gamma$) & 3 & flat to worse & no beneficial trend (small $n$) \\
GroupNorm & 3 & indistinguishable from best unnormalized cell & no beneficial trend (small $n$) \\
Adaptive quantile ceiling & 3 & identical to 4 decimals & no-op (mechanistically proven) \\
Growing schedule ceiling & 10 & identical to 4 decimals, $p{=}1.000$ & no-op (mechanistically proven) \\
EVT-quantile ceiling & 3 & within 0.001--0.006 & no effect detected (small $n$) \\
...
CIFAR-10, $T{=}80$. None reaches significance; the two floor mechanisms and
momentum warm-start move in a consistent positive direction (floor most, by a
clear margin), while the three ceiling mechanisms sit at essentially zero.}
...
longer constrains it. Tying $\tau_t$ to an online estimate of the \emph{current}
...
smoothing over it.} Learning-rate retuning, GroupNorm, and the adaptive-quantile
and EVT-quantile ceilings were tested at $n{=}3$; the growing-schedule ceiling
reached $n{=}10$ on the clean condition but only $n{=}3$ on IPM (the background
job computing it was killed mid-run and superseded by the corrected design
rather than resumed); momentum cold-start, the two floor mechanisms, and the
pretrained-backbone results reached the full $n{=}10$ with paired significance
testing. The DP-noise and non-IID checks, and the floor-clip/local-steps disentangling
ablation (all Section~\ref{sec:resolution}), are $n{=}3$ pilots, the
smallest-$n$ results in the paper to carry this much weight -- appropriate for
a first check of whether the recovery survives at all (it substantially does
not, for DP; substantially degrades, for non-IID Byzantine robustness) or of
which mechanism drives it (floor-clip contributes the large majority of the
combo's gain in both conditions, local steps a smaller minority), but
not for precise magnitudes, and a priority for scaling up before treating
those specific percentages as final.
Table~\ref{tab:audit-summary} states the $n$ used
for every reported number.

\paragraph{The multiple-comparison correction covers the $n{=}10$ family only.}
Section~\ref{sec:setup} applies a Bonferroni correction to the seven
confirmatory ($n{=}10$) $p$-values; none survive, which does not change any
conclusion. The four $n{=}3$ mechanisms (learning-rate and GroupNorm sweeps,
adaptive-quantile and EVT-quantile ceilings) are deliberately excluded from
that correction rather than silently omitted -- two are mechanistically-proven
no-ops for which a $p$-value would be the wrong kind of evidence, and two are
explicitly-labeled sensitivity sweeps, not hypothesis tests. We flag this
scoping choice explicitly rather than let a reader assume the correction covers
every number in Section~\ref{sec:audit}.
...
federated learning algorithm is not, in this case, explained by four
independently-constructed clipping-ceiling mechanisms (one a mechanistically-proven
no-op, confirmed with matched-seed, paired statistical tests; the others tested
at smaller sample sizes and reported as such), by learning-rate transfer or
missing normalization (small, $n{=}3$ sweeps showing no beneficial trend, not
significance-tested nulls), or by momentum cold-start (a small,
