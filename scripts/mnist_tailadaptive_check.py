"""MNIST counterpart of cifar10_tailadaptive_check.py: does TailAdaptiveClip21SGD2M hurt
or help on the dataset where the fixed-tau baseline already works reasonably well? A
genuinely useful mechanism should not regress the easy case even if its biggest upside
is on the hard one. Same protocol (T=80, gamma=0.1, tau_init=1.0, n_regular=20,
SmallCNN-equivalent architecture for MNIST) as the CIFAR-10 check, n=10 seeds, paired
Wilcoxon.
"""
import sys
import os
...
RESULTS_DIR = os.path.join(os.path.dirname(__file__), "..", "results", "mnist_tailadaptive_check")
...
def main():
    ...
    print("\nMNIST tail-adaptive pilot done.", flush=True)
