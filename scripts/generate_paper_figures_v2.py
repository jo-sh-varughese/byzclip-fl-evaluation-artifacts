...
ax.set_xlabel("Accuracy change from each mechanism's own matched baseline (percentage points)", fontsize=9)
...
fig.tight_layout()
fig.savefig(os.path.join(FIG_DIR, "fig_audit_delta.pdf"), bbox_inches="tight")
plt.close(fig)
...
fig.tight_layout()
fig.savefig(os.path.join(FIG_DIR, "fig_resolution_absolute.pdf"), bbox_inches="tight")
plt.close(fig)
...
ax.legend(fontsize=8)
fig.tight_layout()
fig.savefig(os.path.join(FIG_DIR, "fig_round_trace.pdf"), bbox_inches="tight")
plt.close(fig)
